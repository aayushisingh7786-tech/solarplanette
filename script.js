...
// 1. Data Definitions
const planets = [
    { name: "Mercury", distance: 80, radius: 4, color: '#A5A5A5', speed: 0.03, angle: Math.random() * 6, fact: "Smallest planet, closest to Sun. A year is only 88 days!" },
    { name: "Venus", distance: 130, radius: 7, color: '#E3BB76', speed: 0.015, angle: Math.random() * 6, fact: "Earth's twin in size, but hot enough to melt lead on the surface." },
    { name: "Earth", distance: 190, radius: 8, color: '#2271B3', speed: 0.01, angle: Math.random() * 6, fact: "Our home. 70% of the surface is covered by oceans.", hasMoon: true },
    { name: "Mars", distance: 260, radius: 6, color: '#E27B58', speed: 0.008, angle: Math.random() * 6, fact: "The Red Planet. It has the largest volcano in the solar system." },
    { name: "Jupiter", distance: 400, radius: 22, color: '#D39C7E', speed: 0.004, angle: Math.random() * 6, fact: "Gas giant with a 'Great Red Spot' storm larger than Earth." },
    { name: "Saturn", distance: 520, radius: 18, color: '#F4D03F', speed: 0.003, angle: Math.random() * 6, fact: "Its rings are made mostly of ice and rock particles.", hasRings: true }
];
...
function init() {
    ...
    stars = Array.from({ length: 800 }, () => ({ x: Math.random() * width, y: Math.random() * height, size: Math.random() * 1.4, opacity: Math.random() }));
    asteroids = Array.from({ length: 500 }, () => ({
        dist: 290 + Math.random() * 60,
...
// 2. Interaction
function showModal(p) { isPaused = true; document.getElementById('mName').innerText = p.name; document.getElementById('mFact').innerText = p.fact; modal.style.display = 'block'; }
...
// 3. UI Events
document.getElementById('shotBtn').onclick = () => { const link = document.createElement('a'); link.download = 'snapshot.png'; link.href = canvas.toDataURL(); link.click(); };
...
canvas.onclick = () => { const target = getPlanetAtMouse(); if (target) followedPlanet === target ? showModal(target) : followedPlanet = target; };
...
function draw() {
    ...
    ctx.fillStyle = '#FFD700'; ctx.shadowBlur = 60 * viewScale; ctx.shadowColor = '#FFA500';
    ...
    // Comet logic
    if (comet.active) {
        ctx.beginPath(); ctx.strokeStyle = "rgba(255,255,255,0.5)"; ctx.lineWidth = 2 * viewScale;
    ...
    planets.forEach(p => {
        ...
        if (hovered === p || followedPlanet === p) {
            ...
            ctx.fillText(p.name, x, y - (p.radius * viewScale + 25));
        }
